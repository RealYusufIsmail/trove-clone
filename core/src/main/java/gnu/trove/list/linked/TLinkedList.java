 ...
 * <li>objects must only be removed from list they are in. That is, if you have an object A and
 * lists l1 and l2, you must ensure that you invoke List.remove(A) on the correct list.
 * <li>It is also forbidden to invoke List.remove() with an unaffiliated TLinkable (one that belongs
 * to no list): this will destroy the list you invoke it on.
 * </ul>
 *
...
public class TLinkedList<T extends TLinkable<T>> extends AbstractSequentialList<T>
     ...
     * This list will be emptied after the copy (as if clear() had been invoked). The Object[] array
     ...
     * directly.</li>
     * </ol>
     * NOTE: this should only be used with nodes contained in the list. The results are undefined
     * with anything else.
     *
    ...
    protected final class IteratorImpl implements ListIterator<T> {
        ...
        /**
         * returns the index of the next node in the list (the one that would be returned by a call
         * to next()).
         *
         * @return an <code>int</code> value
         */
         ...
         *
         * @throws IllegalStateException neither next nor previous have been invoked, or remove or
         *         add have been invoked after the last invocation of next or previous.
         */
        @SuppressWarnings({"unchecked"})
